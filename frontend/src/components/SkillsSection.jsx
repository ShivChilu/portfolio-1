        ...
        {filteredSkills.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {filteredSkills.map((skill, index) => (
              <div 
                key={index}
                className="p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/50 shadow-sm hover:shadow-md hover:border-red-500/20 dark:hover:border-red-500/20 transition-all duration-200 flex flex-col items-start justify-between min-h-[92px] gap-2 group"
              >
                <span className="text-[9px] uppercase font-bold text-red-600 dark:text-red-400 tracking-wider leading-none">
                  {skill.category}
                </span>
